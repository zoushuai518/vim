...
" F3                         --单源文件编译           [全模式可用 已配置 C Coffee]
" F4                         --单源文件运行           [全模式可用 已配置 C Rb PHP Coffee Bash]
...
colorscheme blackboard       " Ruby蓝色背景
"colorscheme molokai         " Sublime艳丽渲染
"colorscheme tango2          " 浅灰色背景
...
" 每行超过80个的字符用下划线标示
au BufRead,BufNewFile *.c,*.h,*.cl,*.rb,*.php,*.coffee,*.js,*.css,*.sh,*.vim 2match Underlined /.\%81v/
...
" F4 一键保存、运行
map  <f4> :call RunCode()<CR>
imap <f4> <ESC>:call RunCode()<CR>
vmap <f4> <ESC>:call RunCode()<CR>
